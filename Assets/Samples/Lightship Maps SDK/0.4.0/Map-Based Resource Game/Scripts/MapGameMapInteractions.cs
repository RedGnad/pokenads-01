...
namespace Niantic.Lightship.Maps.Samples.GameSample
{
    ...
    internal class MapGameMapInteractions : MonoBehaviour
    {
        ...
        public void StartPlacingStructure(MapGameState.StructureType structureType)
        {
            ...
            Debug.Log($"[MapGameMapInteractions] StartPlacingStructure: StructureType = {_placingStructureType}");
        }
        ...
        private void Update()
        {
            ...
            if (Input.touchCount == 1)
            {
                if (Input.touches[0].phase == TouchPhase.Ended)
                {
                    ...
                    touchDetected = true;
                    Debug.Log($"[MapGameMapInteractions] Touch détecté à {touchPosition}");
                }
            }
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                touchDetected = true;
                Debug.Log($"[MapGameMapInteractions] Clic de souris détecté à {touchPosition}");
            }
            ...
            if (touchDetected)
            {
                ...
                {
                    Debug.Log("[MapGameMapInteractions] Mode de placement activé, appel à PlaceStructure.");
                    PlaceStructure(touchPosition);
                }
                else
                {
                    Debug.Log("[MapGameMapInteractions] Vérification d'une interaction sur un feature.");
        ...
        private LatLng ScreenPointToLatLong(Vector3 screenPosition)
        {
            ...
            Debug.Log($"[MapGameMapInteractions] Conversion de {screenPosition} en LatLng: {latLng}");
            return latLng;
        }

        private void PlaceStructure(Vector3 touchPosition)
        {
            Debug.Log("[MapGameMapInteractions] PlaceStructure appelé.");
            ...
            switch (_placingStructureType)
            {
                case MapGameState.StructureType.Sawmill:
                    Debug.Log("[MapGameMapInteractions] Placement d'une Sawmill.");
                    _sawmillSpawner.PlaceInstance(structureLatLng, rotation);
                    break;

                case MapGameState.StructureType.StoneMason:
                    Debug.Log("[MapGameMapInteractions] Placement d'une StoneMason.");
                ...
                case MapGameState.StructureType.Stronghold:
                    Debug.Log("[MapGameMapInteractions] Placement d'un Stronghold.");
                    _strongholdSpawner.PlaceInstance(structureLatLng, rotation);
                    break;

                default:
                    Debug.LogError("[MapGameMapInteractions] StructureType inconnu lors du placement.");
                    throw new ArgumentOutOfRangeException(nameof(_placingStructureType));
            }

            // Inform the inventory system and others about the building
            MapGameState.Instance.StructureBuilt(structureLatLng, _placingStructureType);
            Debug.Log($"[MapGameMapInteractions] Structure placée à {structureLatLng} de type {_placingStructureType}.");
        ...
        {
            Debug.Log($"[MapGameMapInteractions] CheckForInteractableTouch appelé avec position {touchPosition}");
            var touchRay = _mapCamera.ScreenPointToRay(touchPosition);

            // Raycast into scene to see if we hit a map feature
            if (!Physics.Raycast(touchRay, out var hitInfo))
            {
                Debug.Log("[MapGameMapInteractions] Aucun objet touché par le rayon.");
                return;
            }

            // Check if the hit collider is a resource feature
            var hitResourceItem = hitInfo.collider.GetComponent<MapGameResourceFeature>();
            if (hitResourceItem == null)
            {
                Debug.Log("[MapGameMapInteractions] L'objet touché n'est pas une feature de ressources.");
                return;
            }
            ...
            {
                Debug.Log("[MapGameMapInteractions] Ressources non disponibles sur cet élément.");
                return;
            }

            // Award the player resources and update state
            int amount = hitResourceItem.GainResources();
            MapGameState.Instance.AddResource(hitResourceItem.ResourceType, amount);
            Debug.Log($"[MapGameMapInteractions] Ressources collectées: {amount} de type {hitResourceItem.ResourceType}");

            // Spawn animated floating text to show resources being gained
            ...
            Debug.Log("[MapGameMapInteractions] FloatingText instancié pour afficher les ressources.");
        }
    }
}